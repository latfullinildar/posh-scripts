...
$root_ou.ForEach({Get-ADUser -Credential $RemoteCredentials -Filter * -SearchBase $_.DistinguishedName -Server $remotedc  -properties LastLogonTimestamp,department | Select-Object Name,enabled,distinguishedName,department, @{n='LastLogonTimestamp';e={[DateTime]::FromFileTime($_.LastLogonTimestamp)}} | Export-Csv -NoTypeInformation -Encoding UTF8 -Path "$PSScriptRoot\users\users_$_.Name.csv"})
